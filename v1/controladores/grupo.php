...
class grupo
{
    ...
    const ID_GRUPO = "idGrupo";
    const ID_GANADO = "idGanado";
    const ID_TRABAJADOR = "idTrabajador";
    const NOMBRE_GRUPO = "NombreGrupo";
    ...
    public static function get($peticion)
    {
        ...
        if (empty($peticion[0]))
            return self::obtenerGrupo($idUsuario);
        else
            return self::obtenerGrupo($idUsuario, $peticion[0]);
    ...
    {        
        ...
        $grupo = json_decode($body);
        $idgrupo = especie::crear($grupo);
     ...
     * @throws Exception
     */
    private function obtenerGrupo($idUsuario, $id_tipo = NULL)
    {
        try {
            if (!$id_tipo) {
    ...
    private function crear($grupo)
    {
        if ($grupo) {
            try {
                ...
                $sentencia->bindParam(1, $grupo->idGanado);
                $sentencia->bindParam(2, $grupo->idTrabajador);
                $sentencia->bindParam(3, $grupo->NombreGrupo);
                $sentencia->execute();
     ...
     * @throws Exception
     */
    private function actualizar($grupo, $id_grupo)
    {
        try {
            ...
            $sentencia->bindParam(1, $id_ganado);
            $sentencia->bindParam(2, $id_trabajador);
            $sentencia->bindParam(3, $nombreGrupo);
                        
            $id_ganado = $grupo->idGanado;
            $id_trabajador = $grupo->idTrabajador;
     ...
     */
    private function eliminar($idUsuario, $id_grupo)
    {
        try {
            // Sentencia DELETE
            ...
            $sentencia->bindParam(1, $id_grupo);
